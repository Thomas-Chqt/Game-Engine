...
ImGuiKey glfwKeyToImGuiKey(int keycode, int)
{
    switch (keycode)
    {
...
void Editor::onEvent(GE::Event& event)
{
    ...
    if (event.dispatch<GE::KeyDownEvent>([&](GE::KeyDownEvent& event) {
        ...
        if (event.keyCode() == static_cast<int>(GE::KeyboardButton::one))
            m_viewport.setCamera(&m_editorCamera);
        else if (event.keyCode() == static_cast<int>(GE::KeyboardButton::two))
            m_viewport.setCamera(m_editedScene.second.activeCamera());
        else
            ImGui::GetIO().AddKeyEvent(glfwKeyToImGuiKey(event.keyCode(), 0), true);
    })) return;
    if (event.dispatch<GE::KeyUpEvent>([&](GE::KeyUpEvent& event) {
        ImGui::GetIO().AddKeyEvent(glfwKeyToImGuiKey(event.keyCode(), 0), false);
    })) return;
    if (event.dispatch<GE::MouseDownEvent>([&](GE::MouseDownEvent& event) {
        ImGui::GetIO().AddMouseButtonEvent(event.mouseCode(), true);
    })) return;
    if (event.dispatch<GE::MouseUpEvent>([&](GE::MouseUpEvent& event) {
        ImGui::GetIO().AddMouseButtonEvent(event.mouseCode(), false);
    })) return;
}
